...
| POST | `/api/review/session` | `{limit<=5, entry_point}` | `{session_id, due_total, offered_count, next_due_date, cards[]}` | `services.review_events.create_review_session` |
...
> 最多 5 张。`event_id` 是幂等键——同一 event_id 重试不会重复调度 / boost / 记录。
