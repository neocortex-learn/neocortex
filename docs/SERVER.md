...
| GET | `/api/daily` | — | `DailyBriefing`（Today 最多 3 条 + 总数 + 继续读 1 条 + Top of Mind） | `services.daily.build_briefing` |
| POST | `/api/daily/surface` | `{clip_id, absorbed?}` | `SurfaceUpdate` | `services.daily.mark_surfaced` |
| GET | `/api/inbox` | — | `{items, total}`（只含 `status=inbox`） | `services.inbox.list_inbox` |
| POST | `/api/inbox/action` | `{action_id, clip_id, action, target_action_id?}` | `InboxActionResponse` | `services.inbox.handle_inbox_action` |
| GET | `/api/top-of-mind` | — | `{topics}` | `AppConfig.top_of_mind` |
| PUT | `/api/top-of-mind` | `{topics}` | `{topics}` | `save_config` |
| GET | `/api/map?domain=...&around=...` | query | `ConceptMap`（含 Mermaid 源） | `services.visualize.build_concept_map` |
| POST | `/api/review/session` | `{request_id, limit<=5, entry_point}` | `{session_id, due_total, offered_count, next_due_date, cards[]}` | `services.review_events.create_review_session` |
| POST | `/api/review/action` | `{event_id, action, session_id?, card_id?}` | 按 action 而异（grade 返回新 schedule + session 剩余） | `services.review_events.handle_review_action` |
| POST | `/api/compile` | `{force?}` | `CompileJobStatus`（`accepted=false` 表示已有任务在跑） | `services.compile.compile_notes`（后台任务） |
| GET | `/api/compile/status` | — | `CompileJobStatus`（idle/running/done/failed + current/total 进度） | 同上（轮询快照） |

> Review 约束：`POST /api/review/session` **只允许**在用户明确点击"开始复习"时调用
> （菜单刷新 / daily / 预取用 `GET /api/daily` + `impression`）；服务端强制单次
> 最多 5 张。`request_id` 是 session 创建幂等键，超时重试返回同一 session；
> `event_id` 是 action 幂等键——同一 event_id 重试不会重复调度 / boost / 记录。
> 服务端暂时兼容未传 `request_id` 的上一版已安装 Mac 客户端，但这种 legacy
> 请求不承诺 session 创建幂等；当前客户端必须传入并在超时重试时复用。
> action ∈ `again/hard/good/easy/suspend/restore/open_source/impression`；
> GUI 四档映射 Again→0 / Hard→3 / Good→4 / Easy→5（0/1/2 有意折叠为 Again）。
> 事件与 session 落在 `data/neocortex.sqlite` 的 `review_events` / `review_sessions`
> 表；卡片状态仍在 `vault/.flashcards/*.json`，一致性靠 pending/applied 状态机 +
> 绝对值快照（见 `services/review_events.py` 模块 docstring）。

> 注意：删除笔记走的是 `POST /api/notes/delete`，**不是** `DELETE /api/notes`。
> 这是为了和 4 层安全里的"变更方法强制 JSON body"规则配合。

> Today/Inbox 约束：`GET /api/daily` 的 `surfacing` 只来自已到期且仍为
> `status=inbox` 的内容，最多返回 3 条；`surfacing_total` 是截断前总数。
> `keep/skip/later/master` 分别落到 `reference/archived/later/promoted`，其中
> `reference` 不再主动浮现，`later` 只在 `continue_read` 返回最多 1 条。
> `action_id` 是 Inbox 写入幂等键；`undo` 必须携带指定的
> `target_action_id`，目标之后已有同 clip 新动作时返回 409，不覆盖新状态。
> 事件使用 `data/neocortex.sqlite` 的 `inbox_events` pending/applied 绝对快照；
> clip 更新按实际文件路径原子覆盖，不根据标题重新生成文件名。Top of Mind
> 去空白、大小写不敏感去重，最多 3 个；命中标题、摘要、topic、tag 或相关概念时
> 以 `priority_reason="Top of Mind: <topic>"` 可解释地提升 Today 排序。
